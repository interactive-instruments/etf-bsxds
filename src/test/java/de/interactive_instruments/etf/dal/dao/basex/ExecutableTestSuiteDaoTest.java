...
public class ExecutableTestSuiteDaoTest {
	...
	private void checkLabel(final ExecutableTestSuiteDto dto, final String label, boolean deleted)
		...
		if (deleted) {
			...
			assertNull("Dto " + dto.getId() + " found in collection, but should be disabled",
					collectionResult.get(dto.getId()));
		} else {
			// uniqueness check
			assertTrue(collectionResult.keySet().contains(dto.getId()));
			assertNotNull("Dto " + dto.getId() + " not found in collection", collectionResult.get(dto.getId()));
